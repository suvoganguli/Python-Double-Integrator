...
def solveOptimalControlProblem(system, N, t0, x0, u0, T):
    ...
    # closed-loop control
    sol = minimize(costFunction, u0,
                   args=(runningCosts, system, N, T, t0, x0))  # runningCosts, system, N, T, t0, x0, u0
...
def costFunction(u, runningCosts, system, N, T, t0, x0): # runningCosts, system, N, T, t0, x0, u
    # cost = 0;
    ...
    for k in range(N):
        costvec[k] = runningCosts(t0 + k*T, x[k,:], np.array([u[k]]))
        cost = cost + costvec[k]
...
def computeOpenloopSolution(system, N, T, t0, x0, u):
    ...
    for k in range(N-1):
        x[k+1] = system(x[k], np.array([u[k]]), T)
